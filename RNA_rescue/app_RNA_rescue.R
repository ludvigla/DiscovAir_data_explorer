...
coords.list <- setNames(lapply(paste0("data/coords", c(1:3)), function(f) readRDS(file = f)), nm = paste0(c(1:3)))
expr.data.list <- setNames(lapply(paste0("data/expr.data", c(1:3)), function(f) readRDS(file = f)), nm = paste0(c(1:3)))
expr.data.list <- lapply(expr.data.list, as.matrix)
cell.data.list <- setNames(lapply(paste0("data/cell.data", c(1:3)), function(f) readRDS(file = f)), nm = paste0(c(1:3)))
cell.data.list <- lapply(cell.data.list, function(x) {
  x[is.na(x)] <- 0
  return(x)
})
  ...
  lapply(c(1:3), function(i) {
    output[[paste0("map", i)]] <- renderLeaflet({
      m <- leaflet(
        ...
        setMaxBounds(lng1 = 0, lat1 = 0, lng2 = 594, lat2 = -588) %>% # TODO: find a way to automatically set bounds around tiled image
        addStyleEditor() %>%
        ...
        fitBounds(lng1 = 0, lat1 = 0, lng2 = 594, lat2 = -588)
      return(m)
    })
  })
